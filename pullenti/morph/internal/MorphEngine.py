...
class MorphEngine:
    ...
    def __load_tree_node(self, tn : 'MorphTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos239 = RefOutArgWrapper(pos)
                tn._deserialize_lazy(self.__m_lazy_buf, self, wrappos239)
                pos = wrappos239.value
            tn.lazy_pos = 0
    ...
    def process(self, word : str, ignore_no_dict : bool=False) -> typing.List['MorphWordForm']:
        ...
        while i <= len(word): 
            ...
            if (tn.nodes is None or i >= len(word)): 
                break
            ch = ord(word[i])
            wraptn240 = RefOutArgWrapper(None)
            inoutres241 = Utils.tryGetValue(tn.nodes, ch, wraptn240)
            tn = wraptn240.value
            if (not inoutres241): 
                break
            i += 1
    ...
    def get_all_wordforms(self, word : str) -> typing.List['MorphWordForm']:
        ...
        while i <= len(word): 
            ...
            if (tn.nodes is None or i >= len(word)): 
                break
            ch = ord(word[i])
            wraptn242 = RefOutArgWrapper(None)
            inoutres243 = Utils.tryGetValue(tn.nodes, ch, wraptn242)
            tn = wraptn242.value
            if (not inoutres243): 
                break
            i += 1
    ...
    def get_wordform(self, word : str, cla : 'MorphClass', gender : 'MorphGender', cas : 'MorphCase', num : 'MorphNumber', add_info : 'MorphWordForm') -> str:
        ...
        while i <= len(word): 
            ...
            if (tn.nodes is None or i >= len(word)): 
                break
            ch = ord(word[i])
            wraptn244 = RefOutArgWrapper(None)
            inoutres245 = Utils.tryGetValue(tn.nodes, ch, wraptn244)
            tn = wraptn244.value
            if (not inoutres245): 
                break
            i += 1
    ...
    def deserialize(self, str0 : Stream, ignore_rev_tree : bool, lazy_load : bool) -> None:
        ...
        pos = 0
        wrappos254 = RefOutArgWrapper(pos)
        cou = buf.deserialize_int(wrappos254)
        pos = wrappos254.value
        while cou > 0: 
            mi = MorphMiscInfo()
            wrappos246 = RefOutArgWrapper(pos)
            mi._deserialize(buf, wrappos246)
            pos = wrappos246.value
            self.add_misc_info(mi)
            cou -= 1
        wrappos253 = RefOutArgWrapper(pos)
        cou = buf.deserialize_int(wrappos253)
        pos = wrappos253.value
        while cou > 0: 
            wrappos248 = RefOutArgWrapper(pos)
            p1 = buf.deserialize_int(wrappos248)
            pos = wrappos248.value
            r = MorphRule()
            if (lazy_load): 
                r.lazy_pos = pos
                pos = p1
            else: 
                wrappos247 = RefOutArgWrapper(pos)
                r._deserialize(buf, wrappos247)
                pos = wrappos247.value
            self.add_rule(r)
            cou -= 1
        root = MorphTreeNode()
        if (lazy_load): 
            wrappos249 = RefOutArgWrapper(pos)
            root._deserialize_lazy(buf, self, wrappos249)
            pos = wrappos249.value
        else: 
            wrappos250 = RefOutArgWrapper(pos)
            root._deserialize(buf, wrappos250)
            pos = wrappos250.value
        self.m_root = root
        if (not ignore_rev_tree): 
            root_rev = MorphTreeNode()
            if (lazy_load): 
                wrappos251 = RefOutArgWrapper(pos)
                root_rev._deserialize_lazy(buf, self, wrappos251)
                pos = wrappos251.value
            else: 
                wrappos252 = RefOutArgWrapper(pos)
                root_rev._deserialize(buf, wrappos252)
                pos = wrappos252.value
