...
class LanguageHelper:
    ...
    def get_case_after_preposition(prep : str) -> 'MorphCase':
        mc = None
        wrapmc256 = RefOutArgWrapper(None)
        inoutres257 = Utils.tryGetValue(LanguageHelper.__m_prep_cases, prep, wrapmc256)
        mc = wrapmc256.value
        if (inoutres257): 
            return mc
        else: 
    ...
    def normalize_preposition(prep : str) -> str:
        res = None
        wrapres258 = RefOutArgWrapper(None)
        inoutres259 = Utils.tryGetValue(LanguageHelper.__m_prep_norms, prep, wrapres258)
        res = wrapres258.value
        if (inoutres259): 
            return res
        else: 
            return prep
