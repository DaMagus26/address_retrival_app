...
class GarHelper:
    ...
    def get_object(sid : str) -> 'GarObject':
        if (sid is None or GarHelper.GAR_INDEX is None): 
            return None
        iid = 0
        wrapiid136 = RefOutArgWrapper(0)
        inoutres137 = Utils.tryParseInt(sid[1:], wrapiid136)
        iid = wrapiid136.value
        if (not inoutres137): 
            return None
        if (sid[0] == 'a'): 
            if (iid < 1): 
    ...
    def get_object_params(sid : str) -> typing.List[tuple]:
        if (GarHelper.GAR_INDEX is None): 
            return None
        iid = 0
        wrapiid138 = RefOutArgWrapper(0)
        inoutres139 = Utils.tryParseInt(sid[1:], wrapiid138)
        iid = wrapiid138.value
        if (not inoutres139): 
            return None
        if (sid[0] == 'a'): 
    ...
    def get_children_objects_by_id(sid : str, ignore_houses : bool=False) -> typing.List['GarObject']:
        ...
        res = list()
        iid = 0
        wrapiid140 = RefOutArgWrapper(0)
        inoutres141 = Utils.tryParseInt(sid[1:], wrapiid140)
        iid = wrapiid140.value
        if (not inoutres141): 
            return None
        if (sid[0] == 'a'): 
            ao = GarHelper.GAR_INDEX.getao(iid)
    ...
    def create_addr_object(g : 'GarObject') -> 'AddrObject':
        ...
        elif (g.level == GarLevel.ADMINAREA or g.level == GarLevel.MUNICIPALAREA): 
