    ...
    def __correct_object_by_gars(it : 'AddrObject') -> None:
        ...
        if (len(aa.names) == 0): 
            for g in it.gars: 
                ...
                if (len(ga.names) > 0): 
                    if (aa.number is not None and ga.names[0].find(aa.number) >= 0): 
                        pass
                    else: 
            ...
            if (len(par_ids) == 0): 
                if (i > 0): 
                    ...
                    if (it.level == AddrLevel.CITY and RegionHelper.is_big_citya(it) is not None): 
                        ...
                        continue
                if (AddressHelper.compare_levels(it.level, AddrLevel.LOCALITY) >= 0 or it.level == AddrLevel.SETTLEMENT): 
                    r.strict_search = True
                    if (self.m_params is None): 
                        if (it.level != AddrLevel.LOCALITY and it.level != AddrLevel.SETTLEMENT): 
                            if (it.level == AddrLevel.STREET and i == 0 and self.zip0_ is not None): 
                                pass
                            else: 
                                continue
                        else: 
                            probs2 = GarHelper.GAR_INDEX._get_string_entries(r, regions, par_ids, 3)
                            if (probs2 is not None and len(probs2) == 1): 
                                regions.append(probs2[0].region)
                                search_always = True
                            else: 
                                continue
                    elif (self.m_params.default_object is None): 
    ...
    def __add_miss_items(self, addr : 'TextAddress') -> None:
        ...
        if (len(addr.items) > 0 and len(addr.items[0].gars) >= 1 and len(addr.items[0].gars[0].parent_ids) > 0): 
            ...
            for g in addr.items[0].gars: 
                ta = TextAddress()
                p = self.get_gar_object(g.parent_ids[0])
                while p is not None: 
        ...
        for p in probs: 
            ...
            if (not it.can_be_equals_glevel(g)): 
                if (len(probs) == 1 and it.level == AddrLevel.STREET and g.level == GarLevel.AREA): 
                    pass
                elif (len(probs) == 1 and it.level == AddrLevel.CITY and g.level == GarLevel.LOCALITY): 
                    pass
                elif (len(probs) == 1 and it.level == AddrLevel.TERRITORY and g.level == GarLevel.STREET): 
                    pass
                elif (len(probs) == 1 and it.level == AddrLevel.STREET and g.level == GarLevel.AREA): 
                    pass
                elif ((len(probs) == 1 and it.level == AddrLevel.TERRITORY and g.level == GarLevel.LOCALITY) and "СТ" in aa.miscs and "станица" in ga.types): 
                    it.level = AddrLevel.LOCALITY
                    aa.types.clear()
                    aa.miscs.clear()
    ...
    def _analyze1(self, ar : 'AnalysisResult', txt : str, co : typing.List[tuple], one_addr : bool) -> typing.List['TextAddress']:
        ...
        while True:
            ...
            if (isinstance(t, ReferentToken)): 
                ...
                if ((t.is_value("ПРОП", None) or t.is_value("ПРОПИС", None) or t.is_value("ПРОЖ", None)) or t.is_value("ПРОЖИВ", None)): 
        ...
        if (sec_address and len(res) == 2 and one_addr): 
