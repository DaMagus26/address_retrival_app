...
class RepAddrTree:
    ...
    def open0_(self, dat : bytearray) -> None:
        ...
        while i < cou: 
            ...
            ind += 2
            tn = RepAddrTreeNode()
            wrapind188 = RefOutArgWrapper(ind)
            self.__deserialize_node(tn, wrapind188)
            ind = wrapind188.value
    ...
    def __load_node(self, res : 'RepAddrTreeNode') -> None:
        if (not res.loaded and res.lazy_pos > 0): 
            ind = res.lazy_pos + 4
            wrapind189 = RefOutArgWrapper(ind)
            self.__deserialize_node(res, wrapind189)
            ind = wrapind189.value
        res.loaded = True
    ...
    def find(self, path : str) -> typing.List['RepAddrTreeNodeObj']:
        ...
        while i < len(path): 
            ...
            tn = None
            ch = path[i]
            wraptn190 = RefOutArgWrapper(None)
            inoutres191 = Utils.tryGetValue(dic, ch, wraptn190)
            tn = wraptn190.value
            if (not inoutres191): 
                return None
            if (not tn.loaded): 
    ...
    def add(self, path : str, obj : 'RepAddrTreeNodeObj') -> bool:
        ...
        while i < len(path): 
            tn = None
            ch = path[i]
            wraptn192 = RefOutArgWrapper(None)
            inoutres193 = Utils.tryGetValue(dic, ch, wraptn192)
            tn = wraptn192.value
            if (not inoutres193): 
                tn = RepAddrTreeNode()
                tn.loaded = True
