...
class HouseRoomHelper:
    ...
    def process_house_and_rooms(ah : 'AnalyzeHelper', ar : 'AddressReferent', addr : 'TextAddress') -> None:
            ...
            for kk in range(2):
                for g in ao.gars: 
                    ...
                    if (ah.litera_variant is not None and ar.building is None): 
                        ...
                        hobjs = HouseRoomHelper.__find_houses_new(ah, hinstr, arr, (1 if ao.cross_object is not None else 0), up_num)
                    if (hobjs is None): 
                        if (addr.items[len(addr.items) - 1].level == AddrLevel.STREET): 
                            ...
                            if (aa.number is not None and "блок" in aa.types and ar.box is not None): 
                                ...
                                hobjs = HouseRoomHelper.__find_houses_new(ah, hinstr, ar2, (1 if ao.cross_object is not None else 0), up_num)
                        if (hobjs is None): 
                            hobjs = HouseRoomHelper.__find_houses_new(ah, hinstr, ar, (1 if ao.cross_object is not None else 0), up_num)
                    ar0 = ar
                    if (hobjs is not None): 
                        ...
                        nn = 0
                        if (ii > 0): 
                            wrapnn143 = RefOutArgWrapper(0)
                            Utils.tryParseInt(num[ii + 1:], wrapnn143)
                            nn = wrapnn143.value
                        if (nn > 0 or ((ii > 0 and not hiph))): 
                            ar2 = AddressReferent()
                            ar2.house = num[0:0+ii]
                            ar2.flat = num[ii + 1:]
                            hobjs2 = HouseRoomHelper.__find_houses_new(ah, hinstr, ar2, 0, up_num)
                            ...
                            a1 = AddressReferent()
                            a1.house = num[ii + 1:]
                            hobjs2 = HouseRoomHelper.__find_houses_new(ah, hinstr, a1, (1 if ao.cross_object is not None else 0), up_num)
                        ...
                        elif (ar.plot is not None and ((ar.house is not None or ar.building is not None or ar.corpus is not None))): 
                            ar2 = AddressReferent()
                            ar2.plot = ar.plot
                            hobjs1 = HouseRoomHelper.__find_houses_new(ah, hinstr, ar2, (1 if ao.cross_object is not None else 0), up_num)
                            ar3 = Utils.asObjectOrNull(ar.clone(), AddressReferent)
                            ar3.plot = None
                            hobjs2 = HouseRoomHelper.__find_houses_new(ah, hinstr, ar3, (1 if ao.cross_object is not None else 0), up_num)
                ...
                if (ao.cross_object is not None): 
                    ...
                    for g in ao.cross_object.gars: 
                        ...
                        if (hinstr is None): 
                            continue
                        hobjs = HouseRoomHelper.__find_houses_new(ah, hinstr, ar, 2, None)
        ...
        if (tr is not None and house_gar_objects is not None): 
            ...
            if (len(house_gar_objects) == 1): 
                house = AddrObject._new122(house_gar_objects[0].attrs.clone(), AddrLevel.BUILDING)
        ...
        if (plot is None and ar.plot is not None): 
            plot = AddrObject._new122(HouseAttributes._new154(("б/н" if ar.plot == "0" else ar.plot)), AddrLevel.PLOT)
        if (plot is not None): 
            plot._sort_gars()
            addr.items.append(plot)
        if (house is None and plot is None): 
            if (ar.house is not None): 
                house = AddrObject(HouseAttributes._new156(HouseRoomHelper.__get_house_type(ar.house_type), ("б/н" if ar.house == "0" else ar.house)))
            if (ar.house_or_plot is not None): 
                house = AddrObject(HouseAttributes._new156(HouseType.UNDEFINED, ("б/н" if ar.house_or_plot == "0" else ar.house_or_plot)))
            ...
            if (house is None and ar.box is not None): 
                house = AddrObject(HouseAttributes._new156(HouseType.GARAGE, ("б/н" if ar.box == "0" else ar.box)))
            if (house is None and ar.well is not None): 
                house = AddrObject(HouseAttributes._new156(HouseType.WELL, ("б/н" if ar.well == "0" else ar.well)))
            if (house is not None): 
        ...
        if (house is not None): 
            ...
            if (house.attrs.typ == HouseType.SPECIAL and ar.well is not None): 
                addr.items.append(AddrObject._new122(HouseAttributes._new156(HouseType.WELL, ("б/н" if ar.well == "0" else ar.well)), AddrLevel.BUILDING))
        if (tr is None): 
        ...
        if (tr is not None): 
            ...
            if (ar.carplace is not None and tr.attrs.typ != RoomType.CARPLACE): 
                addr.items.append(AddrObject._new122(RoomAttributes._new164(RoomType.CARPLACE, ar.carplace), AddrLevel.ROOM))
    ...
    @staticmethod
    def __find_houses_new(ah : 'AnalyzeHelper', hinst : 'HousesInStreet', a : 'AddressReferent', cross_num : int, up_num : str) -> typing.List['GarObject']:
        if (a.plot is None or a.house is None or a.plot == a.house): 
            return HouseRoomHelper.__find_houses_new0(ah, hinst, a, cross_num, up_num)
        pl = AddressReferent()
        pl.plot = a.plot
        res1 = HouseRoomHelper.__find_houses_new0(ah, hinst, pl, cross_num, up_num)
        a.plot = None
        res2 = HouseRoomHelper.__find_houses_new0(ah, hinst, a, cross_num, up_num)
        a.plot = pl.plot
        if (res1 is None): 
            return res2
        if (res2 is None): 
    ...
    @staticmethod
    def __find_houses_new0(ah : 'AnalyzeHelper', hinst : 'HousesInStreet', a : 'AddressReferent', cross_num : int, up_num : str) -> typing.List['GarObject']:
        ...
        if (cross_num > 0 and ((a.house is not None or a.house_or_plot is not None))): 
            ...
            if (ii > 0): 
                ...
                res1 = HouseRoomHelper.__find_houses_new0(ah, hinst, ar1, 0, None)
                if (res1 is not None): 
                    if (cross_num == 1): 
                        res0 = HouseRoomHelper.__find_houses_new0(ah, hinst, a, 0, None)
        ...
        if (a.corpus_or_flat is not None): 
            ...
            res1 = HouseRoomHelper.__find_houses_new0(ah, hinst, aa1, cross_num, None)
            aa1.corpus = None
            res2 = HouseRoomHelper.__find_houses_new0(ah, hinst, aa1, cross_num, None)
            if (res1 is not None): 
        ...
        if (a.house is not None and a.house.find('-') > 0): 
            ...
            if (Utils.compareStrings(a1.house, a2.house, False) < 0): 
                res1 = HouseRoomHelper.__find_houses_new0(ah, hinst, a1, 0, None)
                res2 = HouseRoomHelper.__find_houses_new0(ah, hinst, a2, 0, None)
    ...
    def try_parse_list_items(ah : 'AnalyzeHelper', addr : 'TextAddress', ar : 'AnalysisResult') -> bool:
        ...
        if (house is not None): 
            if (house.plot_number is not None): 
                ...
                Utils.tryParseInt(house.plot_number, wrapn0166)
                n0 = wrapn0166.value
                ...
                Utils.tryParseInt(house.number, wrapn0167)
                n0 = wrapn0167.value
            elif (house.build_number is not None): 
                if (house.stroen_number is not None): 
                    return False
                wrapn0168 = RefOutArgWrapper(0)
                Utils.tryParseInt(house.build_number, wrapn0168)
                n0 = wrapn0168.value
            elif (house.stroen_number is not None): 
                wrapn0169 = RefOutArgWrapper(0)
                inoutres170 = Utils.tryParseInt(house.stroen_number, wrapn0169)
                n0 = wrapn0169.value
                if (not inoutres170): 
            ...
            wrapn0171 = RefOutArgWrapper(0)
            Utils.tryParseInt(room.number, wrapn0171)
            n0 = wrapn0171.value
        b0 = t.begin_char
        ar0 = None
        try: 
            ar0 = ProcessorService.get_empty_processor().process(SourceOfAnalysis._new128(addr.text[b0:], "ADDRESS"), None, None)
        ...
        while True:
            ...
            if (not ok): 
                break
            n1 = 0
            wrapn1173 = RefOutArgWrapper(0)
            Utils.tryParseInt(ait.value, wrapn1173)
            n1 = wrapn1173.value
        ...
        for n in nums: 
            ...
            if (room is not None): 
                ...
                if (house.plot_number is not None): 
                    ...
                    a.building = n
                gg = HouseRoomHelper.__find_houses_new(ah, hinstr, a, 0, None)
                if (gg is not None): 
                    it.gars.extend(gg)
        return True
    ...
    def try_process_details(addr : 'TextAddress', details : str) -> None:
        if (Utils.isNullOrEmpty(details) or addr.last_item is None or addr.last_item.detail_typ != DetailType.UNDEFINED): 
            return
        try: 
            ar0 = ProcessorService.get_empty_processor().process(SourceOfAnalysis._new128(details, "ADDRESS"), None, None)
            t = ar0.first_token
            first_pass3162 = True
            while True:
