...
class AddrObject:
    ...
    def can_be_equals_glevel(self, gar : 'GarObject') -> bool:
        ...
        if (AddressHelper.can_be_equal_levels(self.level, gar.level)): 
            return True
        if (self.level == AddrLevel.LOCALITY and gar.level == GarLevel.SETTLEMENT): 
            return True
    ...
    def clone(self) -> 'AddrObject':
        ...
        res.tag = self.tag
        res.rep_object = self.rep_object
        if (self.cross_object is not None): 
    ...
    @staticmethod
    def _new122(_arg1 : 'BaseAttributes', _arg2 : 'AddrLevel') -> 'AddrObject':
