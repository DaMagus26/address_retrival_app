...
class StringDictionaryTable(BaseTable):
    ...
    def __restore(self, data : bytearray) -> None:
        ind = 0
        while ind < len(data):
            wrapind3119 = RefOutArgWrapper(ind)
            s = BaseTable.get_string_for_bytes(data, wrapind3119, False, None)
            ind = wrapind3119.value
            if (s is None): 
                break
    ...
    def get_code_by_string(self, val : str, add_if_not_exist : bool) -> int:
        if (Utils.isNullOrEmpty(val)): 
            return 0
        id0_ = 0
        wrapid3120 = RefOutArgWrapper(0)
        inoutres3121 = Utils.tryGetValue(self.__m_hash, val, wrapid3120)
        id0_ = wrapid3120.value
        if (inoutres3121): 
            return id0_
