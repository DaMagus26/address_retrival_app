...
class KeyBaseTable(BaseTable):
    ...
    def get_unique_key(self) -> int:
        ...
        while True:
            ...
            wrapdisp3098 = RefOutArgWrapper(0)
            wraplen3099 = RefOutArgWrapper(0)
            inoutres3100 = self.__read_index_info(self.__m_unique_key_position, wrapdisp3098, wraplen3099)
            disp = wrapdisp3098.value
            len0_ = wraplen3099.value
            if (not inoutres3100): 
                continue
            if (disp == (0) and len0_ == 0): 
                retVal3541 = self.__m_unique_key_position
                self.__m_unique_key_position += 1
                return retVal3541
        self.__m_unique_key_position = -1
        return max0_ + 1
    ...
    def read_key_data_len(self, key : int) -> int:
        ...
        disp = 0
        len0_ = 0
        wrapdisp3101 = RefOutArgWrapper(0)
        wraplen3102 = RefOutArgWrapper(0)
        inoutres3103 = self.__read_index_info(key, wrapdisp3101, wraplen3102)
        disp = wrapdisp3101.value
        len0_ = wraplen3102.value
        if (not inoutres3103): 
            return -1
        else: 
            return len0_
    ...
    def read_key_data(self, key : int, max_len : int=0) -> bytearray:
        ...
        disp = 0
        len0_ = 0
        wrapdisp3104 = RefOutArgWrapper(0)
        wraplen3105 = RefOutArgWrapper(0)
        inoutres3106 = self.__read_index_info(key, wrapdisp3104, wraplen3105)
        disp = wrapdisp3104.value
        len0_ = wraplen3105.value
        if (not inoutres3106): 
            if (log0_): 
    ...
    def remove_key_data(self, key : int) -> None:
        ...
        disp = 0
        len0_ = 0
        wrapdisp3107 = RefOutArgWrapper(0)
        wraplen3108 = RefOutArgWrapper(0)
        inoutres3109 = self.__read_index_info(key, wrapdisp3107, wraplen3108)
        disp = wrapdisp3107.value
        len0_ = wraplen3108.value
        if (not inoutres3109): 
            return
        if (disp == (0)): 
            return
    ...
    def update_part_of_data(self, key : int, data : bytearray, pos : int) -> None:
        disp = 0
        len0_ = 0
        wrapdisp3110 = RefOutArgWrapper(0)
        wraplen3111 = RefOutArgWrapper(0)
        inoutres3112 = self.__read_index_info(key, wrapdisp3110, wraplen3111)
        disp = wrapdisp3110.value
        len0_ = wraplen3111.value
        if (not inoutres3112): 
            return
    ...
    def update_start_of_data(self, key : int, data : bytearray) -> None:
        disp = 0
        len0_ = 0
        wrapdisp3113 = RefOutArgWrapper(0)
        wraplen3114 = RefOutArgWrapper(0)
        inoutres3115 = self.__read_index_info(key, wrapdisp3113, wraplen3114)
        disp = wrapdisp3113.value
        len0_ = wraplen3114.value
        if (not inoutres3115): 
            return
        self._m_data.position = disp
        self._m_data.write(data, 0, len(data))
    ...
    def __add_data(self, key : int, data : bytearray, dst : Stream) -> None:
        ...
        if (dst == self._m_data): 
            disp = 0
            len0_ = 0
            wrapdisp3116 = RefOutArgWrapper(0)
            wraplen3117 = RefOutArgWrapper(0)
            inoutres3118 = self.__read_index_info(key, wrapdisp3116, wraplen3117)
            disp = wrapdisp3116.value
            len0_ = wraplen3117.value
            if (inoutres3118): 
