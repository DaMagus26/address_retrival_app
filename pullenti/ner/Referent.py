...
class Referent:
    ...
    def get_int_value(self, attr_name : str, def_value : int) -> int:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return def_value
        res = 0
        wrapres3044 = RefOutArgWrapper(0)
        inoutres3045 = Utils.tryParseInt(str0_, wrapres3044)
        res = wrapres3044.value
        if (not inoutres3045): 
            return def_value
        return res
    ...
    def add_occurence_of_ref_tok(self, rt : 'ReferentToken') -> None:
        self.add_occurence(TextAnnotation._new1124(rt.kit.sofa, rt.begin_char, rt.end_char, rt.referent))
    ...
    def add_occurence(self, anno : 'TextAnnotation') -> None:
        ...
        if (anno.occurence_of != self and anno.occurence_of is not None): 
            anno = TextAnnotation._new3047(anno.begin_char, anno.end_char, anno.sofa)
        if (self.__m_occurrence is None): 
            self.__m_occurrence = list()
    ...
    def clone(self) -> 'Referent':
        ...
        for r in self.slots: 
            rr = Slot._new3048(r.type_name, r.value, r.count)
            rr.owner = res
            res.slots.append(rr)
        return res
    ...
    def deserialize(self, stream : Stream, all0_ : typing.List['Referent'], sofa : 'SourceOfAnalysis', new_format : bool=False) -> None:
        ...
        i = 0
        while i < cou: 
            a = TextAnnotation._new3049(sofa, self)
