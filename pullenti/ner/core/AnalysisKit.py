...
class AnalysisKit:
        ...
        if (tokens is not None): 
            ...
            while ii < len(tokens): 
                ...
                if (sofa_.correction_dict is not None): 
                    corw = None
                    wrapcorw828 = RefOutArgWrapper(None)
                    inoutres829 = Utils.tryGetValue(sofa_.correction_dict, mt.term, wrapcorw828)
                    corw = wrapcorw828.value
                    if (inoutres829): 
                        ccc = MorphologyService.process(corw, lang, None)
                        if (ccc is not None and len(ccc) == 1): 
                            tt1 = TextToken._new827(ccc[0], self, tt.begin_char, tt.end_char, tt.term)
                            tt1.chars = tt.chars
                            tt = tt1
    ...
    def correct_word_by_morph(self, tt : 'Token', lang : 'MorphLang') -> 'Token':
        ...
        if (ccc is None or len(ccc) != 1): 
            return None
        tt1 = TextToken._new830(ccc[0], self, tt.begin_char, tt.end_char, tt.chars, tt.term)
    ...
    def embed_token(self, mt : 'MetaToken') -> None:
        ...
        if (isinstance(mt, ReferentToken)): 
            if (mt.referent is not None): 
                mt.referent.add_occurence(TextAnnotation._new831(self.sofa, mt.begin_char, mt.end_char))
    ...
    def get_analyzer_data(self, analyzer : 'Analyzer') -> 'AnalyzerData':
        ...
        if (analyzer is None or analyzer.name is None): 
            return None
        d = None
        wrapd832 = RefOutArgWrapper(None)
        inoutres833 = Utils.tryGetValue(self.__m_datas, analyzer.name, wrapd832)
        d = wrapd832.value
        if (inoutres833): 
            d.kit = self
            return d
    ...
    def deserialize(self, stream : Stream) -> bool:
        ...
        self.base_language = MorphLang._new287(SerializerHelper.deserialize_int(stream))
        self.__m_entities = list()
    ...
    @staticmethod
    def _new3033(_arg1 : 'Processor', _arg2 : 'ExtOntology') -> 'AnalysisKit':
        res = AnalysisKit()
        res.processor = _arg1
        res.ontology = _arg2
        return res
    ...
    @staticmethod
    def _new3034(_arg1 : 'SourceOfAnalysis', _arg2 : bool, _arg3 : 'MorphLang', _arg4 : EventHandler, _arg5 : 'ExtOntology', _arg6 : 'Processor', _arg7 : bool) -> 'AnalysisKit':
