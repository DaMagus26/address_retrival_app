...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new751(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refresh_generals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new751(r, ad)
                all_refs.append(nod)
                r.tag = (nod)
                si = None
                wrapsi754 = RefOutArgWrapper(None)
                inoutres755 = Utils.tryGetValue(all0_, a.name, wrapsi754)
                si = wrapsi754.value
                if (not inoutres755): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    if (s is None): 
                        continue
                    li = [ ]
                    wrapli752 = RefOutArgWrapper(None)
                    inoutres753 = Utils.tryGetValue(si, s, wrapli752)
                    li = wrapli752.value
                    if (not inoutres753): 
                        li = list()
                        si[s] = li
