...
class NumberHelper:
    ...
    def __try_parse(token : 'Token', prev_val : 'NumberToken'=None) -> 'NumberToken':
        ...
        if (val is not None): 
                ...
                if (str.isdigit(sss[0]) and len(sss) == 3): 
                    ...
                    while True:
                        ...
                        if (ttt.whitespaces_before_count == 1 and ttt.length_char == 3 and str.isdigit(ss[0])): 
                            ii = 0
                            wrapii867 = RefOutArgWrapper(0)
                            inoutres868 = Utils.tryParseInt(ss, wrapii867)
                            ii = wrapii867.value
                            if (not inoutres868): 
                                break
                            val2 += ss
            ...
            for k in range(4):
                if ((isinstance(et.next0_, TextToken)) and et.next0_.chars.is_letter): 
                    ...
                    break
            res = NumberToken._new869(token, et, val, typ, mc)
        ...
        while t is not None: 
            ...
            if (num is None): 
                break
            wrapj870 = RefOutArgWrapper(0)
            inoutres871 = Utils.tryParseInt(str(num.termin.tag), wrapj870)
            j = wrapj870.value
            if (not inoutres871): 
                break
    ...
    def try_parse_age(t : 'Token') -> 'NumberToken':
            ...
            if (nt_next is not None): 
                ...
                if (isinstance(t1, TextToken)): 
                    ...
                    if ((v == "ЛЕТ" or v == "ЛЕТИЯ" or v == "ЛЕТИЕ") or v == "РІЧЧЯ" or v == "ЖЫЛЖЫГЫ"): 
                        return NumberToken._new869(t, t1, nt.value, NumberSpellingType.AGE, t1.morph)
        ...
        if (LanguageHelper.ends_with_ex(s, "ЛЕТИЕ", "ЛЕТИЯ", "РІЧЧЯ", None)): 
            ...
            if (term is not None): 
                return NumberToken._new869(tt, tt, str(term.tag), NumberSpellingType.AGE, tt.morph)
        s = tt.lemma
        i = s.find("ЛЕТН")
        if (i < 0): 
            i = s.find("РІЧН")
        if (i > 3): 
            ...
            for i in range(len(ss) - 2, 2, -1):
                ...
                if (len(num0) > 1 and num0[len(num0) - 1] == '0' and len(num1) == 1): 
                    return NumberToken._new869(tt, tt, num0[0:0+len(num0) - 1] + num1, NumberSpellingType.AGE, tt.morph)
            else: i = 2
        return None
    ...
    def __analize_number_tail(tt : 'TextToken', val : str) -> 'MetaToken':
            ...
            if (((s == "<" or s == "(")) and (isinstance(tt.next0_, TextToken))): 
                ...
                if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
                    if (tt.next0_.next0_ is not None and tt.next0_.next0_.is_char_of(">)")): 
                        ...
                        return MetaToken._new877(tt, tt.next0_.next0_, mc)
            return None
        if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
            mc = MorphCollection()
            mc.class0_ = MorphClass.ADJECTIVE
            mc.language = MorphLang.EN
            return MetaToken._new877(tt, tt, mc)
        ...
        if (tt.morph.language.is_ua and mc is None and s == "Ї"): 
            mc = MorphCollection()
            mc.add_item(MorphBaseInfo._new880(MorphClass.ADJECTIVE))
        if (mc is not None): 
            return MetaToken._new877(tt, tt, mc)
        if ((((len(s) < 3) and not tt.is_whitespace_before and tt.previous is not None) and tt.previous.is_hiphen and not tt.previous.is_whitespace_before) and tt.whitespaces_after_count == 1 and s != "А"): 
            if (len(s) == 1 and s != "Й" and not LanguageHelper.is_cyrillic_vowel(s[0])): 
                ...
                return MetaToken._new877(tt, tt, MorphCollection._new882(MorphClass.ADJECTIVE))
        return None
    ...
    def try_parse_real_number(t : 'Token', attrs : 'NumberParseAttr'=NumberParseAttr.NO) -> 'NumberToken':
        ...
        if (t.next0_ is not None and t.next0_.is_value("ЦЕЛЫЙ", "ЦІЛИЙ")): 
            ...
            if ((isinstance(tt1, NumberToken)) or (((isinstance(tt1, TextToken)) and tt1.is_value("НОЛЬ", "НУЛЬ")))): 
                ...
                if (res0.real_value == 0): 
                    ...
                    dd = 0
                    wrapdd886 = RefOutArgWrapper(0)
                    inoutres887 = Utils.tryParseFloat(str0_, wrapdd886)
                    dd = wrapdd886.value
                    if (inoutres887): 
                        pass
                    else: 
                        wrapdd884 = RefOutArgWrapper(0)
                        inoutres885 = Utils.tryParseFloat(str0_.replace('.', ','), wrapdd884)
                        dd = wrapdd884.value
                        if (inoutres885): 
                            pass
                        else: 
                ...
                return res0
        d = 0
        wrapd889 = RefOutArgWrapper(0)
        tt = NumberHelper.__try_parse_float(Utils.asObjectOrNull(t, NumberToken), wrapd889, attrs)
        d = wrapd889.value
        if (tt is None): 
        ...
        if (tt.next0_ is not None and tt.next0_.is_value("ДЕСЯТОК", None)): 
            d *= (10)
            tt = tt.next0_
        return NumberExToken._new888(t0, tt, "", NumberSpellingType.DIGIT, NumberExType.UNDEFINED, d)
    ...
    def get_number_base_info(num_val : str, cla : 'MorphClass') -> 'MorphBaseInfo':
        ...
        Utils.tryParseInt(num_val, wrapn892)
        n = wrapn892.value
        res = MorphBaseInfo._new880(cla)
    ...
    def string_to_double(str0_ : str) -> float:
        ...
        if (str0_ == "NaN"): 
            return math.nan
        wrapres897 = RefOutArgWrapper(0)
        inoutres898 = Utils.tryParseFloat(str0_, wrapres897)
        res = wrapres897.value
        if (inoutres898): 
            return res
        wrapres895 = RefOutArgWrapper(0)
        inoutres896 = Utils.tryParseFloat(str0_.replace('.', ','), wrapres895)
        res = wrapres895.value
        if (inoutres896): 
            return res
        wrapres893 = RefOutArgWrapper(0)
        inoutres894 = Utils.tryParseFloat(str0_.replace(',', '.'), wrapres893)
        res = wrapres893.value
        if (inoutres894): 
            return res
        return None
