...
class ShortNameHelper:
    ...
    class ShortnameVar:
        ...
        @staticmethod
        def _new2785(_arg1 : str, _arg2 : 'MorphGender') -> 'ShortnameVar':
    ...
    def get_names_for_shortname(shortname : str) -> typing.List['ShortnameVar']:
        res = [ ]
        wrapres2783 = RefOutArgWrapper(None)
        inoutres2784 = Utils.tryGetValue(ShortNameHelper.M_SHORTS_NAMES, shortname, wrapres2783)
        res = wrapres2783.value
        if (not inoutres2784): 
            return None
        else: 
            return res
        ...
        if (obj is not None): 
            ...
            while t is not None: 
                if (t.is_newline_before): 
                    ...
                    for s in shos: 
                        li = None
                        wrapli2786 = RefOutArgWrapper(None)
                        inoutres2787 = Utils.tryGetValue(ShortNameHelper.M_SHORTS_NAMES, s, wrapli2786)
                        li = wrapli2786.value
                        if (not inoutres2787): 
                            li = list()
                            ShortNameHelper.M_SHORTS_NAMES[s] = li
                        li.append(ShortNameHelper.ShortnameVar._new2785(nam, g))
                    if (t is None): 
                        break
                    t = t.previous
                t = t.next0_
