...
class DateReferent(Referent):
    ...
    def calculate_date_range(self, now : datetime.datetime, from0_ : datetime.datetime, to : datetime.datetime, tense : int=0) -> bool:
        ...
        """
        from pullenti.ner.date.internal.DateRelHelper import DateRelHelper
        inoutres1213 = DateRelHelper.calculate_date_range(self, now, from0_, to, tense)
        return inoutres1213
        ...
        if (self.is_relative): 
            ...
            for s in self.slots: 
                if (s.type_name == DateReferent.ATTR_CENTURY): 
                    word = "век"
                    wrapval1215 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1215)
                    val = wrapval1215.value
                elif (s.type_name == DateReferent.ATTR_DECADE): 
                    word = "декада"
                    wrapval1216 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1216)
                    val = wrapval1216.value
                elif (s.type_name == DateReferent.ATTR_HALFYEAR): 
                    word = "полугодие"
                    wrapval1217 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1217)
                    val = wrapval1217.value
                elif (s.type_name == DateReferent.ATTR_SEASON): 
                    wrapval1218 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1218)
                    val = wrapval1218.value
                    if (val == 1): 
                    ...
                    word = "год"
                    wrapval1219 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1219)
                    val = wrapval1219.value
                elif (s.type_name == DateReferent.ATTR_MONTH): 
                    word = "месяц"
                    wrapval1220 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1220)
                    val = wrapval1220.value
                    ...
                    word = "день"
                    wrapval1221 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1221)
                    val = wrapval1221.value
                    ...
                    word = "квартал"
                    wrapval1222 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1222)
                    val = wrapval1222.value
                elif (s.type_name == DateReferent.ATTR_WEEK): 
                    word = "неделя"
                    wrapval1223 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1223)
                    val = wrapval1223.value
                elif (s.type_name == DateReferent.ATTR_HOUR): 
                    word = "час"
                    wrapval1224 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1224)
                    val = wrapval1224.value
                    if (not is_local_rel): 
                        print("{0}:{1}".format("{:02d}".format(val), "{:02d}".format(self.minute)), end="", file=res, flush=True)
                elif (s.type_name == DateReferent.ATTR_MINUTE): 
                    word = "минута"
                    wrapval1225 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1225)
                    val = wrapval1225.value
                elif (s.type_name == DateReferent.ATTR_DAYOFWEEK): 
                    wrapval1226 = RefOutArgWrapper(0)
                    Utils.tryParseInt(Utils.asObjectOrNull(s.value, str), wrapval1226)
                    val = wrapval1226.value
                    if (not is_local_rel): 
    ...
    @staticmethod
    def _new1126(_arg1 : 'DateReferent', _arg2 : int) -> 'DateReferent':
        res = DateReferent()
        res.higher = _arg1
        res.day = _arg2
        return res
    ...
    @staticmethod
    def _new1127(_arg1 : int, _arg2 : int) -> 'DateReferent':
        res = DateReferent()
        res.month = _arg1
        res.day = _arg2
        return res
    ...
    @staticmethod
    def _new1128(_arg1 : int) -> 'DateReferent':
        res = DateReferent()
        res.year = _arg1
        return res
    
    @staticmethod
    def _new1133(_arg1 : int, _arg2 : int) -> 'DateReferent':
        res = DateReferent()
        res.hour = _arg1
        res.minute = _arg2
        return res
    ...
    @staticmethod
    def _new1135(_arg1 : 'DatePointerType') -> 'DateReferent':
        res = DateReferent()
        res.pointer = _arg1
        return res
    
    @staticmethod
    def _new1147(_arg1 : int, _arg2 : 'DateReferent') -> 'DateReferent':
        res = DateReferent()
        res.month = _arg1
        res.higher = _arg2
        return res
    ...
    @staticmethod
    def _new1152(_arg1 : int, _arg2 : 'DateReferent') -> 'DateReferent':
        res = DateReferent()
        res.day = _arg1
        res.higher = _arg2
        return res
    ...
    @staticmethod
    def _new1165(_arg1 : int) -> 'DateReferent':
        res = DateReferent()
        res.month = _arg1
        return res
    
    @staticmethod
    def _new1167(_arg1 : int, _arg2 : int, _arg3 : 'DateReferent') -> 'DateReferent':
        res = DateReferent()
        res.hour = _arg1
        res.minute = _arg2
        res.higher = _arg3
    ...
    @staticmethod
    def _new1185(_arg1 : int, _arg2 : bool) -> 'DateReferent':
        res = DateReferent()
        res.century = _arg1
    ...
    @staticmethod
    def _new1186(_arg1 : int, _arg2 : bool) -> 'DateReferent':
        res = DateReferent()
        res.decade = _arg1
        res.is_relative = _arg2
        return res
    ...
    @staticmethod
    def _new1193(_arg1 : int) -> 'DateReferent':
        res = DateReferent()
        res.day = _arg1
        return res
    
    @staticmethod
    def _new1195(_arg1 : 'DateReferent') -> 'DateReferent':
        res = DateReferent()
        res.higher = _arg1
        return res
    
    @staticmethod
    def _new1196(_arg1 : 'DateReferent', _arg2 : int) -> 'DateReferent':
        res = DateReferent()
        res.higher = _arg1
        res.month = _arg2
        return res
    ...
    @staticmethod
    def _new1206(_arg1 : int) -> 'DateReferent':
        res = DateReferent()
