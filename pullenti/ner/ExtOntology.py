...
class ExtOntology:
    ...
    def add(self, ext_id : object, type_name : str, definition_ : str) -> 'ExtOntologyItem':
        ...
        if (rs is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3001(ext_id, rs[0], type_name)
        if (len(rs) > 1): 
            del rs[0]
            res._refs = rs
    ...
    def add_referent(self, ext_id : object, referent : 'Referent') -> 'ExtOntologyItem':
        ...
        if (referent is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3001(ext_id, referent, referent.type_name)
        self.items.append(res)
        return res
    
    def __create_referent(self, type_name : str, definition_ : str) -> typing.List['Referent']:
        analyzer = None
        wrapanalyzer3003 = RefOutArgWrapper(None)
        inoutres3004 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapanalyzer3003)
        analyzer = wrapanalyzer3003.value
        if (not inoutres3004): 
            return None
    ...
    def refresh(self, item : 'ExtOntologyItem', definition_ : object) -> bool:
        ...
        analyzer = None
        wrapanalyzer3005 = RefOutArgWrapper(None)
        inoutres3006 = Utils.tryGetValue(self.__m_anal_by_type, item.type_name, wrapanalyzer3005)
        analyzer = wrapanalyzer3005.value
        if (not inoutres3006): 
            return False
    ...
    def _get_analyzer_data(self, type_name : str) -> 'AnalyzerData':
        a = None
        wrapa3007 = RefOutArgWrapper(None)
        inoutres3008 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapa3007)
        a = wrapa3007.value
        if (not inoutres3008): 
            return None
    ...
    def __init_hash(self) -> None:
        with self.__m_lock: 
            ...
            for it in self.items: 
                if (it.referent is not None): 
                    ont = None
                    wrapont3010 = RefOutArgWrapper(None)
                    inoutres3011 = Utils.tryGetValue(hash0_, it.referent.type_name, wrapont3010)
                    ont = wrapont3010.value
                    if (not inoutres3011): 
                        ont = IntOntologyCollection._new3009(True)
    ...
    def attach_referent(self, r : 'Referent') -> typing.List['ExtOntologyItem']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3012 = RefOutArgWrapper(None)
        inoutres3013 = Utils.tryGetValue(self.__m_hash, r.type_name, wraponto3012)
        onto = wraponto3012.value
        if (not inoutres3013): 
            return None
        li = [ ]
    ...
    def attach_token(self, type_name : str, t : 'Token') -> typing.List['IntOntologyToken']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3014 = RefOutArgWrapper(None)
        inoutres3015 = Utils.tryGetValue(self.__m_hash, type_name, wraponto3014)
        onto = wraponto3014.value
        if (not inoutres3015): 
            return None
