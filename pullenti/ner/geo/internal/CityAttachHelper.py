...
class CityAttachHelper:
    ...
    def try_define(li : typing.List['CityItemToken'], ad : 'GeoAnalyzerData', always : bool=False) -> 'ReferentToken':
        ...
        if (res is None and len(li) > 1): 
            ...
            if (res is not None and res.end_char <= li[1].end_char): 
                res = (None)
        if (res is None): 
            wrapoi1237 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try1(li, wrapoi1237, ad)
            oi = wrapoi1237.value
        if (res is None): 
            wrapoi1238 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_noun_name(li, wrapoi1238, False)
            oi = wrapoi1238.value
        if (res is None): 
            wrapoi1239 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_name_exist(li, wrapoi1239, False)
            oi = wrapoi1239.value
        if (res is None): 
            res = CityAttachHelper.__try4(li)
        if (res is None and always): 
            wrapoi1240 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_noun_name(li, wrapoi1240, True)
            oi = wrapoi1240.value
        if (res is None and always): 
            if (OrgItemToken.try_parse(li[0].begin_token, ad) is not None): 
                pass
            else: 
                wrapoi1241 = RefOutArgWrapper(None)
                res = CityAttachHelper.__try_name_exist(li, wrapoi1241, True)
                oi = wrapoi1241.value
    ...
    def __try1(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', ad : 'AnalyzerDataWithOntology') -> 'ReferentToken':
        ...
        if (oi.value is None): 
            ...
            rt = ReferentToken._new1146(city, li[0].begin_token, li[len(li) - 1].end_token, mc)
        elif (typ is None): 
            typ = oi.value.typ
        if (rt is None): 
    ...
    def __try_noun_name(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', always : bool) -> 'ReferentToken':
            ...
            if (not li[0].doubtful): 
                ...
                if (npt is not None): 
                    if (npt.end_token.end_char > li[i1].end_char and len(npt.adjectives) > 0 and not npt.adjectives[0].end_token.next0_.is_comma): 
                        ...
                        if (len(li2) > 1): 
                            oi2 = None
                            wrapoi21245 = RefOutArgWrapper(None)
                            inoutres1246 = CityAttachHelper.__try_noun_name(li2, wrapoi21245, False)
                            oi2 = wrapoi21245.value
                            if (inoutres1246 is not None): 
                                ok = True
        ...
        res = ReferentToken._new1146(city, li[0].begin_token, li[len(li) - 1].end_token, mc)
        num = None
    ...
    def __try_name_exist(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', always : bool) -> 'ReferentToken':
        ...
        city._add_misc(li[0].misc)
        city._add_misc(li[0].misc2)
        return ReferentToken._new1146(city, li[0].begin_token, Utils.ifNotNull(end_tok, li[0].end_token), li[0].morph)
