...
class PhoneHelper:
    ...
    def initialize() -> None:
        ...
        for line0 in Utils.splitString(str0_, '\n', False): 
            ...
            while i < len(cod): 
                dig = cod[i]
                nn = None
                wrapnn2812 = RefOutArgWrapper(None)
                inoutres2813 = Utils.tryGetValue(tn.children, dig, wrapnn2812)
                nn = wrapnn2812.value
                if (not inoutres2813): 
    ...
    def get_country_prefix(full_number : str) -> str:
        ...
        while i < len(full_number): 
            dig = full_number[i]
            nn = None
            wrapnn2814 = RefOutArgWrapper(None)
            inoutres2815 = Utils.tryGetValue(nod.children, dig, wrapnn2814)
            nn = wrapnn2814.value
            if (not inoutres2815): 
                break
