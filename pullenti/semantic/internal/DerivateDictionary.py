...
class DerivateDictionary:
    ...
    def __load_tree_node(self, tn : 'ExplanTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos3075 = RefOutArgWrapper(pos)
                tn._deserialize(self.__m_buf, self, True, wrappos3075)
                pos = wrappos3075.value
            tn.lazy_pos = 0
    ...
    def _deserialize(self, str0_ : Stream, lazy_load : bool) -> None:
        ...
        with MemoryStream() as tmp: 
            ...
            pos = 0
            wrappos3079 = RefOutArgWrapper(pos)
            cou = wr.deserialize_int(wrappos3079)
            pos = wrappos3079.value
            while cou > 0: 
                wrappos3077 = RefOutArgWrapper(pos)
                p1 = wr.deserialize_int(wrappos3077)
                pos = wrappos3077.value
                ew = DerivateGroup()
                if (lazy_load): 
                    ew._lazy_pos = pos
                    pos = p1
                else: 
                    wrappos3076 = RefOutArgWrapper(pos)
                    ew._deserialize(wr, wrappos3076)
                    pos = wrappos3076.value
                ew.id0_ = (len(self._m_all_groups) + 1)
                self._m_all_groups.append(ew)
                cou -= 1
            self._m_root = ExplanTreeNode()
            wrappos3078 = RefOutArgWrapper(pos)
            self._m_root._deserialize(wr, self, lazy_load, wrappos3078)
            pos = wrappos3078.value
        self.__m_buf = wr
